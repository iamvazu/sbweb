...
create table public.bids (
  ...
  buyer_email             text,
  portal_link             text,
  prebid_type             text check (prebid_type in 
                          ('M','NM',null)),
...
create table public.user_bid_matches (
  ...
  pipeline_stage text default 'new_match'
                 check (pipeline_stage in (
                   'new_match','reviewing','pursuing',
                   'submitted','won','lost','passed'
                 )),
  user_decision  text check (user_decision in 
                 ('pursue','monitor','pass',null)),
...
create policy "Users can insert own matches"
  on public.user_bid_matches for insert
  with check (auth.uid() = user_id);

create policy "Users can update own matches"
  on public.user_bid_matches for update
  using (auth.uid() = user_id);
...
create policy "Users can insert own engagements"
  on public.service_engagements for insert
  with check (auth.uid() = user_id);
...
create index matches_fit_score_idx 
  on public.user_bid_matches(fit_score desc);
create index matches_stage_idx 
  on public.user_bid_matches(pipeline_stage);
