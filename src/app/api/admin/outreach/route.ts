...
export async function POST(req: Request) {
  try {
    ...
    if ((bid.dvbe_goal && bid.dvbe_goal !== "0") || bid.event_name?.toLowerCase().includes('dvbe') || bid.comments?.toLowerCase().includes('dvbe')) {
      if (cert_types?.includes('DVBE')) {
        template = 1;
      }
    } else if (bid.sbe_only || bid.event_name?.toLowerCase().includes('small business') || bid.comments?.toLowerCase().includes('sbe')) {
